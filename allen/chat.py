...
def build_system(
    ...
    system += (
        ...
        "• Promotion (STRICT GATE — all three conditions required before you ADD): only write a NEW memory when "
        "(1) it is NET-NEW — not already covered by a memory listed above (check first; if it duplicates or only "
        "refines an existing one, UPDATE that id instead of adding a near-duplicate); (2) it is genuinely "
        "advisable to keep — a stable fact, standing rule, commitment, or decision worth retaining, never passing "
        "chatter or anything you can re-derive; AND (3) Rahm has EXPLICITLY asked you to remember/save it. If any "
        "one of the three is missing, do NOT add — at most ask whether he wants it kept. This keeps his memory "
        "deliberate and clutter-free.\n"
