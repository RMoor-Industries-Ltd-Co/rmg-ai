...
| `allen/logs.py` | Fault-log off-loader — hourly job that appends the onboard `error_log` (auto-captured tool faults + ALLEN's own `log_error` calls) to a rolling per-namespace file in the ALLEN/LOGS Drive folder (self-heals the folder, tail-capped), then marks rows offloaded. Read via `read_error_log` tool or `/admin/errors`; paired with the HONESTY directive so ALLEN records a fault + tells Rahm instead of claiming a false 'done' |
